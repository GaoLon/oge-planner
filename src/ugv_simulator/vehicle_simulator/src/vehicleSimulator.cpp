...
void speedHandler(const ackermann_msgs::AckermannDriveStamped cmd)
{	
  vehicleSpeed = cmd.drive.speed;
  vehicleYawRate = vehicleSpeed * tan(cmd.drive.steering_angle) / wheelbase;
}
...
int main(int argc, char** argv)
{
  ...
  nhPrivate.getParam("wheelbase", wheelbase);

  ros::Subscriber subTerrainCloud = nh.subscribe<sensor_msgs::PointCloud2>("/terrain_map", 2, terrainCloudHandler);

  // ros::Subscriber subSpeed = nh.subscribe<geometry_msgs::TwistStamped>("/command", 5, speedHandler);
  ros::Subscriber subSpeed  = nh.subscribe( "/car/ackermann", 1,  speedHandler );

  ros::Publisher pubVehicleOdom = nh.advertise<nav_msgs::Odometry>("/viscar/odom", 5);
  ...
  while (status)
  {
    ...
    vehicleYaw += 0.01 * vehicleYawRate;
    if (vehicleYaw > PI)
      vehicleYaw -= 2 * PI;
    else if (vehicleYaw < -PI)
      vehicleYaw += 2 * PI;

    if (vehicleYawRate>1e-4)
    {
      vehicleX += vehicleSpeed/vehicleYawRate*(sin(vehicleYaw+0.01*vehicleYawRate)-sin(vehicleYaw));
      vehicleY += vehicleSpeed/vehicleYawRate*(cos(vehicleYaw)-cos(vehicleYaw+0.01*vehicleYawRate));
    }else
    {
      vehicleX += 0.01 * cos(vehicleYaw) * vehicleSpeed +
                0.01 * vehicleYawRate * (-sin(vehicleYaw) * sensorOffsetX - cos(vehicleYaw) * sensorOffsetY);
      vehicleY += 0.01 * sin(vehicleYaw) * vehicleSpeed +
                0.01 * vehicleYawRate * (cos(vehicleYaw) * sensorOffsetX - sin(vehicleYaw) * sensorOffsetY);
    }
    ...
    // publish 100Hz odometry messages
    ...
    odomData.twist.twist.angular.x = 100.0 * (vehicleRoll - vehicleRecRoll);
    odomData.twist.twist.angular.y = 100.0 * (vehiclePitch - vehicleRecPitch);
    odomData.twist.twist.angular.z = vehicleYawRate;
    odomData.twist.twist.linear.x = vehicleSpeed*cos(vehicleYaw);
    odomData.twist.twist.linear.y = vehicleSpeed*sin(vehicleYaw);
    odomData.twist.twist.linear.z = 0.0;
    // odomData.twist.twist.linear.z = 100.0 * (vehicleZ - vehicleRecZ);
