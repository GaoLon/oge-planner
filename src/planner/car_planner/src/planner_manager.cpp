...
namespace ego_planner
{
  ...
  void EGOPlannerManager::initPlanModules(ros::NodeHandle &nh, PlanningVisualization::Ptr vis)
  {
    ...
    nh.param("manager/min_R", pp_.min_R_, -1.0);
    ...
    do
    {
      ...
      //   }
        if (true)
        {
          ...
          Eigen::Vector3d Op;
          Eigen::Vector3d ry;
          if ((local_target_pt-start_pt).norm()>2*pp_.min_R_)
          {
            if ((local_target_pt-start_pt).dot(now_y)>0)
            {
              O = start_pt + pp_.min_R_ * now_y;
              ry = -now_y;
            }
            else
            {
            ...
            Op = local_target_pt - O;
            if (now_y[0]*Op[1]-now_y[1]*Op[0]>0)
            {
              theta = 2*M_PI - acos(ry.dot(Op)/Op.norm()) - acos(pp_.min_R_/Op.norm());
            }
            else
            {
              theta = acos(ry.dot(Op)/Op.norm()) - acos(pp_.min_R_/Op.norm());
            }
            int dint = theta>M_PI_4?ceil(theta/M_PI_4):0;
            double dtheta  = theta/dint;
            for (int i=1;i<=dint;i++)
            {
              double the = i*dtheta;
              points.push_back(O+pp_.min_R_*(sin(the)*now_x+cos(the)*ry));
            }
          }
          ...
          {
            pos.col(i) = points[i];
            real_d += (points[i]-points[i+1]).norm();
          }
                                         ...
                                         const Eigen::Vector3d &end_pos, const Eigen::Vector3d &end_vel, const Eigen::Vector3d &end_acc)
  {
    ...
    Eigen::Vector3d Op;
    Eigen::Vector3d ry;
    if ((end_pos-start_pos).norm()>2*pp_.min_R_)
    {
      if ((end_pos-start_pos).dot(now_y)>0)
      {
        O = start_pos + pp_.min_R_ * now_y;
        ry = -now_y;
      }
      else
      {
      ...
      for (int i=1;i<=dint;i++)
      {
        double the = i*dtheta;
        points.push_back(O+pp_.min_R_*(sin(the)*now_x+cos(the)*ry));
        // ROS_WARN("SHITTING POINT=%f, %f",points.back()[0],points.back()[1]);
      }
    }
