...
namespace ego_planner
{
  ...
  void BsplineOptimizer::setParam(ros::NodeHandle &nh)
  {
    ...
    nh.param("optimization/max_kappa", max_kappa_, -1.0);
  ...
  void BsplineOptimizer::setEnvironment(const TratMap::Ptr &map)
  {
    this->trat_map_ = map;
  }

  void BsplineOptimizer::setEnvironment(const TratMap::Ptr &map, const fast_planner::ObjPredictor::Ptr mov_obj)
  {
    this->trat_map_ = map;
    this->moving_objs_ = mov_obj;
  }
  ...
  std::vector<ControlPoints> BsplineOptimizer::distinctiveTrajs(vector<std::pair<int, int>> segments)
  {
    ...
    const double RESOLUTION = trat_map_->getResolution();
    ...
    for (int i = 0; i < seg_upbound; i++)
    {
      ...
      if (RichInfoSegs[i].first.size > 1)
      {
        ...
        for (int j = 0; j < RichInfoSegs[i].first.size - 1; j++)
        {
          ...
          for (double a = 1; a > 0; a -= step_size)
          {
            ...
            if (trat_map_->getInflateOccupancy(pt))
            {
              occ_start_id = j;
              occ_start_pt = pt;
      ...
      exit_multi_loop2:;
        ...
        for (int j = occ_start_id; j <= occ_end_id; j++)
        {
          ...
          if (trat_map_->getInflateOccupancy(base_pt_reverse)) // Search outward.
          {
            ...
            for (; l <= l_upbound; l += RESOLUTION)
            {
              ...
              if (!trat_map_->getInflateOccupancy(base_pt_temp))
              {
  ...
  std::vector<std::pair<int, int>> BsplineOptimizer::initControlPoints(Eigen::MatrixXd &init_points, bool flag_first_init /*= true*/)
  {
    ...
    double step_size = trat_map_->getResolution() / ((init_points.col(0) - init_points.rightCols(1)).norm() / (init_points.cols() - 1)) / 1.5;
    ...
    for (int i = order_; i <= i_end; ++i)
    {
      ...
      for (double a = 1.0; a > 0.0; a -= step_size)
      {
        ...
        occ = trat_map_->getInflateOccupancy(a * init_points.col(i - 1) + (1 - a) * init_points.col(i));
        //cout << " " << occ;
        // cout << setprecision(5);
      ...
      for (int j = segment_ids[i].first + 1; j < segment_ids[i].second; ++j)
      {
        ...
        if (got_intersection_id >= 0)
        {
          ...
          if (length > 1e-5)
          {
            ...
            //   }
            // }
            for (double a = length; a >= 0.0; a -= trat_map_->getResolution())
            {
              occ = trat_map_->getInflateOccupancy((a / length) * intersection_point + (1 - a / length) * init_points.col(j));

              if (occ || a < trat_map_->getResolution())
              {
                if (occ)
                  a += trat_map_->getResolution();
                cps_.base_point[j].push_back((a / length) * intersection_point + (1 - a / length) * init_points.col(j));
                cps_.direction[j].push_back((intersection_point - init_points.col(j)).normalized());
                // cout << "A " << j << endl;
                break;
  ...
  void BsplineOptimizer::calcCurvatureCost(const Eigen::MatrixXd &q, double &cost, Eigen::MatrixXd &gradient)
  {
    cost = 0.0;
    Eigen::Vector3d delta_now, delta_next;
    double norm_now, norm_next, inv_norm_now, inv_norm_next;
    double delta_phi;

    for (int i = 0; i < q.cols() - 2; i++)
    {
      delta_now = q.col(i+1) - q.col(i);
      delta_next = q.col(i+2) - q.col(i+1);
  ...
  bool BsplineOptimizer::check_collision_and_rebound(void)
  {
    ...
    for (int i = order_ - 1; i <= i_end; ++i)
    {
      ...
      bool occ = trat_map_->getInflateOccupancy(cps_.points.col(i));
      ...
      if (occ)
      {
        for (size_t k = 0; k < cps_.direction[i].size(); ++k)
        {
          ...
          if ((cps_.points.col(i) - cps_.base_point[i][k]).dot(cps_.direction[i][k]) < 1 * trat_map_->getResolution()) // current point is outside all the collision_points.
          {
      ...
      if (occ)
      {
        ...
        for (j = i - 1; j >= 0; --j)
        {
          ...
          occ = trat_map_->getInflateOccupancy(cps_.points.col(j));
          if (!occ)
          {
            in_id = j;
            break;
        ...
        for (int j = segment_ids[i].first + 1; j < segment_ids[i].second; ++j)
        {
          ...
          if (got_intersection_id >= 0)
          {
            ...
            if (length > 1e-5)
            {
              ...
              for (double a = length; a >= 0.0; a -= trat_map_->getResolution())
              {
                bool occ = trat_map_->getInflateOccupancy((a / length) * intersection_point + (1 - a / length) * cps_.points.col(j));
  ...
  bool BsplineOptimizer::rebound_optimize(double &final_cost)
  {
    ...
    do
    {
          ...
          result == lbfgs::LBFGS_STOP)
      {
        ...
        double t_step = (tmp - tm) / ((traj.evaluateDeBoorT(tmp) - traj.evaluateDeBoorT(tm)).norm() / trat_map_->getResolution());
        for (double t = tm; t < tmp * 2 / 3; t += t_step) // Only check the closest 2/3 partition of the whole trajectory.
        {
          // flag_occ = grid_map_->getInflateOccupancy(traj.evaluateDeBoorT(t));
          flag_occ = trat_map_->getInflateOccupancy(traj.evaluateDeBoorT(t));
          if (flag_occ)
          {
  ...
  bool BsplineOptimizer::refine_optimize()
  {
    ...
    do
    {
      ...
      for (double t = tm; t < tmp * 2 / 3; t += t_step)
      {
        ...
        if (trat_map_->getInflateOccupancy(traj.evaluateDeBoorT(t)))
        {
